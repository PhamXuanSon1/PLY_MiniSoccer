...
public class SpriteSizeAdapter : MonoBehaviour
{
    ...
    /// <summary>
    /// Thay đổi Sprite mới và tự động căn chỉnh kích thước chuẩn
    /// </summary>
    /// <param name="newSprite">Sprite cầu thủ/hình ảnh mới</param>
    ...
    /// <summary>
    /// Thực hiện căn chỉnh kích thước theo sprite hiện tại
    /// </summary>
    ...
    {
        if (_uiImage == null || _uiImage.sprite == null || _rectTransform == null) return;
        if (targetSize.x <= 0 || targetSize.y <= 0) return;
    ...
    {
        if (!showGizmos || targetSize.x <= 0 || targetSize.y <= 0) return;

        Gizmos.color = gizmoColor;

        // Nếu là SpriteRenderer trong 2D/3D Scene
    ...
    #region Static Helper Methods (Tiện ích gọi nhanh từ bất kỳ script nào)

    /// <summary>
    /// Helper: Gán sprite và tự động scale SpriteRenderer theo kích thước mong muốn mà không cần gắn component
    /// </summary>
    public static void SetSpriteRendererSize(SpriteRenderer sr, Sprite sprite, Vector2 targetSize, FitMode mode = FitMode.FitInside)
    {
        if (sr == null) return;
        sr.sprite = sprite;
        if (sprite == null || targetSize.x <= 0 || targetSize.y <= 0) return;
        ...
        float chosenScale = (mode == FitMode.FitInside) ? Mathf.Min(scaleX, scaleY) :
                            (mode == FitMode.Envelope) ? Mathf.Max(scaleX, scaleY) : scaleX;
