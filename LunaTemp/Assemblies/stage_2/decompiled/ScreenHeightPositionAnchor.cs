...
public class ScreenHeightPositionAnchor : MonoBehaviour
{
	...
	{
		if (realignOnScreenSizeChanged && (lastScreenWidth != Screen.width || lastScreenHeight != Screen.height || HasCameraChanged()))
		{
			AlignToScreenHeightRatio();
			CacheScreenState();
		}
	...
	public void AlignToScreenHeightRatio()
	{
		...
		if (!(cameraToUse == null))
		{
			...
			base.transform.position += targetWorldPosition - anchor.position;
		}
	}
